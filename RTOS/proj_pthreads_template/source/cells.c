...
//border cells conditions
#define LEFT_NEIGHBOUR_ROW(r) (r == 0) ? (config_NE - 1) : (r - 1)//top
#define RIGHT_NEIGHBOUR_ROW(r) (r == config_NE - 1) ? (0) : (r + 1)//bottom
#define TOP_NEIGHBOUR_COL(c) (c == 0) ? (config_ME - 1) : (c - 1)//left
#define BOTTOM_NEIGHBOUR_COL(c) (c == config_ME - 1) ? (0) : (c + 1)//right
/*
 * PRIVATE FUNCTIONS
 */
...
size_t countLiveNeighbours(size_t row, size_t col)
{
	...
	neighbour_arr[f_posn] = env[LEFT_NEIGHBOUR_ROW(row)][BOTTOM_NEIGHBOUR_COL(
			col)];
	neighbour_arr[g_posn] = env[row][BOTTOM_NEIGHBOUR_COL(col)];
	neighbour_arr[h_posn] = env[RIGHT_NEIGHBOUR_ROW(row)][BOTTOM_NEIGHBOUR_COL(
			col)];
