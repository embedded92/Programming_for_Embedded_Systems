...
#define GPIO_INTERRUPT_PIN_1    17	/* GPIO pin number mapped to interrupt */
#define GPIO_INTERRUPT_PIN_2    18	/* GPIO pin number mapped to interrupt */
 ...
 ****************************************************************************/
uint32_t LED_status_GREEN = 1; // 1==off and 0==on
uint32_t LED_status_BLUE = 1; //1==off and 0==on
...
void GPIO_IRQ_HANDLER(void) {
	...
	interrupt_status = Chip_GPIOINT_GetStatusFalling(LPC_GPIOINT,
	GPIO_INTERRUPT_PORT);

	//state_1=interrupt_status && 1<< GPIO_INTERRUPT_PIN_1;
	if ((interrupt_status >> GPIO_INTERRUPT_PIN_1) == 1) {

		if (LED_status_BLUE == 1)	//if LED_status_BLUE is off
				{
			if (LED_status_GREEN == 0)	//if state of green led is ON
					{
				Board_LED_Set(1, 1); //set green led OFF
				LED_status_GREEN = 1;
	...
	if ((interrupt_status >> GPIO_INTERRUPT_PIN_2) == 1) {

		if (LED_status_GREEN == 1)	//if LED_status_BLUE is off
				{
			if (LED_status_BLUE == 0)	//if state of green led is ON
					{
				Board_LED_Set(2, 1); //set green led OFF
				LED_status_BLUE = 1;
	...
	Chip_GPIOINT_ClearIntStatus(LPC_GPIOINT, GPIO_INTERRUPT_PORT,
			1 << GPIO_INTERRUPT_PIN_1);
	Chip_GPIOINT_ClearIntStatus(LPC_GPIOINT, GPIO_INTERRUPT_PORT,
			1 << GPIO_INTERRUPT_PIN_2);
	//Board_LED_Toggle(0);
}
/**
...
int main(void) {
	...
	Chip_GPIOINT_SetIntFalling(LPC_GPIOINT, GPIO_INTERRUPT_PORT,
			(1 << GPIO_INTERRUPT_PIN_1) | (1 << GPIO_INTERRUPT_PIN_2));
